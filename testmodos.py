...
class TestModosApp(App):
    ...
    def build(self):
        return Builder.load_file('testmodos.kv')
    ...
    def ejecutar_open_loop(self, tiempo_raw):
        if not self.conectado or not self.ser:
            print("Error: Arduino no conectado")
            return

        try:
            # 1. Preparar datos (Modo 2 para Open Loop)
            modo = 2 
            tsim = abs(int(tiempo_raw)) 
            ...
            # 3. Enviar Tiempo de Simulación
            self.ser.write(f"{tsim}\n".encode())
            print(f"Enviado Modo {modo} (Open Loop) y Tiempo {tsim}")

            # 4. Esperar confirmación
            respuesta = self.ser.readline().decode('utf-8').strip()
            if respuesta:
                print(f"Arduino confirma Open Loop: {respuesta}")
            else:
                print("Aviso: No se recibió respuesta en Open Loop")

        except ValueError:
            print("Error: El tiempo debe ser un número entero")
        except Exception as e:
            print(f"Error en Open Loop: {e}")
